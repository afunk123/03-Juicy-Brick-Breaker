...
func _physics_process(_delta):
	if dying and not $Taxis.emitting:
		queue_free()

func hit(_ball):
	var brick_sound = get_node_or_null("/root/Game/Square_Brick")
	if brick_sound != null:
		brick_sound.play()
	die()
...
func die():
	...
	$Tween.interpolate_property($SBrick, "modulate:a", 1.0, 0.0, time_rotate, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.start()
